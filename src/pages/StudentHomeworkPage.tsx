...
export function StudentHomeworkPage() {
  ...
  const group = currentStudent ? state.groups.find((item) => item.id === currentStudent.groupId) : undefined;

  useEffect(() => {
    if (!currentStudent || !canUseApi || !token) {
      setTasks([]);
      setSpeakingTasks([]);
      return;
    }
  ...
  }, [canUseApi, currentStudent, token]);
